...
pub fn render(title: &str, body: &str) -> Vec<u8> {
    ...
    ext(
        &mut body_bytes,
        format!(
            "<< /Title ({}) /Author (Adithya N Raj) /Creator (kronrod {}) /Producer (kronrod pdf module, Rust std-only) /CreationDate (D:20260101000000+00'00') /ModDate (D:20260101000000+00'00') >>\nendobj\n",
            esc(&format!("Kronrod Verification Dossier v{}", crate::VERSION)),
            crate::VERSION
        ),
    );
    ...
    // Fixed /ID (ASCII "KRONROD-DOSSIER-1.0.0" and "...-1.0.0-2" hex) —
    ...
    ext(
        &mut body_bytes,
        format!("{info_obj} 0 R /ID [<4b524f4e442d444f53534945522d312e302e30> <4b524f4e442d444f53534945522d312e302e302d32>] >>\n"),
    );
...
pub fn render_dossier(report: &str) -> Vec<u8> {
    render(
        &format!("KRONROD VERIFICATION DOSSIER v{}", crate::VERSION),
        report,
    )
}
